...
void test6()
{
	...
	// on attributes of all three types (int, double, string)
	std::cout << "---------------------" << std::endl;
	std::cout << "Entering test6 : Reopening" << std::endl;
	std::cout << "---------------------" << std::endl;
	std::cout << "createRelationForward" << std::endl;
	createRelationForward();
	reopenTests();
	deleteRelation();
	std::cout << "Passeed test6 : Reopening" << std::endl;
}
...
void test8()
{
	...
	// on attributes of all three types (int, double, string)
	std::cout << "---------------------" << std::endl;
	std::cout << "Entering test8 : Insert Sparce keys" << std::endl;
	std::cout << "---------------------" << std::endl;
	std::cout << "createRelationSparce" << std::endl;
	createRelationSparce();
	sparceTests();
	deleteRelation();
	std::cout << "Passeed test8 : Sparce keys" << std::endl;
}
...
void createRelationSparce()
{
  ...
  // insert records with sparce number, the diff of two key is 5

  std::vector<int> intvec(3000);
  int curr = 0;
   for(int i = 0; i < 3000; i++ )
	{
    sprintf(record1.s, "%05d string record", curr);
    record1.i = curr;
    record1.d = (double)curr;
	curr +=5 ;
    std::string new_data(reinterpret_cast<char*>(&record1), sizeof(record1));
...
void intReopen()
{
  std::cout << "Reopening an index on an existing one." << std::endl;
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	std::cout << "Passed Reopening test." << std::endl;
}
...
{
  std::cout << "Reopening an index on an existing one." << std::endl;
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,s), STRING);
...
void intSparce()
{
  std::cout << "Testing on sparce relations" << std::endl;
  BTreeIndex index(relationName, doubleIndexName, bufMgr, offsetof(tuple,i), INTEGER);
...
{
  std::cout << "Testing on sparce relations" << std::endl;
  BTreeIndex index(relationName, doubleIndexName, bufMgr, offsetof(tuple,s), STRING);
...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,300,GT,400,LT), 99)
	checkPassFail(intScan(&index,3000,GTE,4000,LT), 1000)
	std::cout << "Running Test 5: Search for key from -1000 to 6000" << std::endl;
